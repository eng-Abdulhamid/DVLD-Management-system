...
namespace Services
{
    ...
    public class PersonServices
    {
        ...
        {
            return _GetResultFromGetPeopleList(repo.GetPeople(_MapToRepoSearchCriteria(SearchCriteria)));
        }
        public bool IsPersonIDExists(int PersonID)
        {
            return repo.IsPersonExistByPersonID(PersonID);
        }
        public int AddNew(PersonAddDTO AddDTO)
        {
            if (!_ValidationBeforeAddNew(AddDTO)) return -1;
            int AddResult = repo.AddNewPerson(_MapAddDTOToEntity(AddDTO));
            if (AddResult > 0)
            {
                return AddResult;
            }
        ...
        {
            return repo.GetPeopleCount(_MapToRepoSearchCriteria(SearchCriteria));
        }
        public OperationResult<PersonReadDTO> FindByPersonID(int PersonID)
        {
            var data = repo.FindPersonByPersonID(PersonID);
        ...
        {
            if (repo.DeletePersonByPersonID(PersonID))
            {
                return true;
            }
            else
        ...
        {
            return repo.UpdatePersonByPersonID(_MapUpdateDTOToEntity(UpdatedData));
        }
        public OperationResult<PersonReadDTO> FindByNationalNo(string NationalNo)
        {
            var data = repo.FindPersonByNationalNo(NationalNo);
            if (data == null) return OperationResult<PersonReadDTO>.FailureDBAError(enResult.rDBAError);
            // simple not-found check: if primary numeric and <=0 treat as not found else if all default treat not found
            bool notFound = false;
            if (notFound) return OperationResult<PersonReadDTO>.Failure(enResult.rNotFound, "No Person Data Found.");
        ...
        {
            if (repo.DeletePersonByNationalNo(NationalNo))
            {
                return true;
            }
            else
        ...
        {
            return repo.IsPersonExistByNationalNo(NationalNo);
        }
        public bool UpdateByNationalNo(PersonUpdateDTO UpdatedData)
        {
            return repo.UpdatePersonByNationalNo(_MapUpdateDTOToEntity(UpdatedData));
        }
        #endregion
        #region Validations
