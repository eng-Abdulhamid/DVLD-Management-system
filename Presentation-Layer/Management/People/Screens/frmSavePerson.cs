...
{
    public partial class frmSavePerson : BaseForm
    {
        ...
        private async Task PopulateInitialDropdowns()
        {
        ...
        private async Task LoadCountries()
        {
            ...
            var results = await countryService.GetAllAsync();

            if (results.IsSuccess && results.DataList.Count > 0)
            {
                foreach (CountryReadDTO country in results.DataList)
                {
        ...
        private bool IsThereAnyUpdate()
        {
            return !((txtAddress.Text == _existingPerson?.Address) &&
                (txtPhone.Text == _existingPerson.Phone) &&
                (txtEmail.Text == _existingPerson.Email) &&
                (txtFirstName.Text == _existingPerson.FirstName) &&
                (txtSecondName.Text == _existingPerson.SecondName) &&
                ...
                (txtLastName.Text == _existingPerson.LastName) &&
                (txtNationalNo.Text == _existingPerson.NationalNo) &&
                (DateTime.Parse(dtpBirthDate.Text) == _existingPerson.DateOfBirth) &&
                (cmbGender.Text == _existingPerson.Gendor.ToString()) &&
                (cmbCountry.Text == _existingPerson.CountryName) &&
                (pbPersonPhoto.Tag.ToString() == _existingPerson.ImagePath));
        }
        private async Task PerformAddPersonAsync(Gendor selectedGender, string photoPath)
        {
        ...
        private async Task PerformUpdatePersonAsync(Gendor selectedGender, string photoPath)
        {
        ...
        private async Task PerformSaveAsync()
        {
            ...
            if (_mode == Mode.AddNew)
            {
                PerformAddPersonAsync(selectedGender, photoPath);
            }
            else
            {
                PerformUpdatePersonAsync(selectedGender, photoPath);
            }
